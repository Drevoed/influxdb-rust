...
use crate::query::{InfluxDbQuery, Timestamp, ValidQuery};
...
impl InfluxDbWriteQuery {
    ...
    ///
    /// ```rust
    /// use influxdb::query::{InfluxDbQuery, Timestamp, create_write_query};
    ///
    /// let _ = create_write_query(Timestamp::NOW, "measurement").add_field("field1", 5).build();
    /// ```
    ...
    /// use influxdb::query::{InfluxDbQuery, Timestamp, create_write_query};
    ///
    /// let _ = create_write_query(Timestamp::NOW, "measurement")
...
impl From<InfluxDbType> for String {
    fn from(db_type: InfluxDbType) -> Self {
        use InfluxDbType::*;
